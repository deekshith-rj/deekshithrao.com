...
export default function Projects({ title, text, projects, meta }) {
  ...
  const makeProjectRows = (projects, columnCount) => {
    const featuredProjects = projects.filter(
      (project) => project.featured_overview
    );
      ...
      <div className="flex flex-col gap-y-16 xs:gap-y-20">
        ...
        {rowsTwoColumns.map((row, index) => (
          <div
            key={index}
            className={`${
              row[0].featured_overview ? "grid-cols-1" : "grid-cols-2"
        ...
        {rowsThreeColumns.map((row, index) => (
          <div
            key={index}
            className={`${
              row[0].featured_overview ? "grid-cols-1" : "grid-cols-3"
        ...
        {rowsFourColumns.map((row, index) => (
          <div
            key={index}
            className={`${
              row[0].featured_overview ? "grid-cols-1" : "grid-cols-4"
